...
with open(output_csv_path, "w", newline='') as csv_file:
    ...
    header = ["Archivo de Audio", "n_mels", "hop_length", "Umbral", "Grupos de Columnas de Ceros"]
    ...
    with open(output_txt_path, "w") as txt_file:
        for audio_file in audio_files:
            ...
            tamano_ven = int(0.02 * sr)
            hop_length_variants = [tamano_ven // 4, tamano_ven // 2, (tamano_ven // 4) * 3]

            txt_file.write(f"Procesando archivo: {audio_file}\n")

            for n_mels in n_mels_range:
                for hop_length in hop_length_variants:
                    mel_spectrogram = librosa.feature.melspectrogram(y=y, sr=sr, n_fft=tamano_ven, hop_length=hop_length, n_mels=n_mels, window='hann')
                    ...
                    for threshold in threshold_range:
                        ...
                        if len(zero_column_indices) > 0:
                            ...
                            if start_index <= end_index:
                                ...
                                txt_file.write(f"  Procesando con hop_length = {hop_length}\n")
                                txt_file.write(f"    Procesando con n_mels = {n_mels}\n")
                                txt_file.write(f"      Umbral = {threshold}\n")
                                txt_file.write(f"      Se encontraron {num_groups} grupos de columnas de ceros en el rango central.\n")
                                row = [audio_file, n_mels, hop_length, threshold, num_groups]
                            else:
                                txt_file.write(f"    No hay columnas de ceros en el rango central.\n")
                                row = [audio_file, n_mels, hop_length, threshold, "0"]
                        else:
                            txt_file.write(f"    No se encontraron columnas de ceros.\n")
                            row = [audio_file, n_mels, hop_length, threshold, "No se encontraron columnas de ceros."]
                        ...
                        axs[1, 0].set_title(f"Image with Threshold and Segments")
                        ...
                        plt.tight_layout()
                        plt.savefig(f"{audio_file}_n_mels_{n_mels}_hop_{hop_length}_umbral_{threshold}.png")
                        plt.close()
